...
class CourseCertificationCourseController extends CoreController
{
    ...
    public function __construct(Course $model)
    {
        ...
        if (is_array($this->breadcrumb) && count($this->breadcrumb) > 0) {
            $this->breadcrumb[0]->title = " Certifications ";
        }

        $this->permitted_actions = [
            'index'  => CourseCertificationPermission::COURSE_INDEX,
            'create' => CourseCertificationPermission::COURSE_CREATE,
            'edit'   => CourseCertificationPermission::COURSE_EDIT,
            'delete' => CourseCertificationPermission::COURSE_DELETE,
        ];
    }

    public function callbackQuery($query)
    {
        return $query->CourseCertification();
    }
...
//            'cyberq_course_id' => ['required', 'max:255'],
        ...
        if (!$this->request->challenge_id) {
            $this->request->request->add(['challenge_id' => null]);
        }
...
//            'cyberq_course_id' => ['required', 'max:255'],
            ...
            'internal_name'    => ['required', 'max:255'],
            'activation'       => ['required'],
            'slug_url'         => ['required', "unique:courses,slug_url,{$this->request->course_certification_course}", 'max:255', new SlugRule],
            'price'            => ['required', 'numeric', 'max:9999', 'min:0'],
            'slack_url'        => 'url'
        ]);
    ...
    private function sharedFormData()
    {
        ...
        $challenges = Challenge::active()->pluck('name', 'id');
        return view()->share(compact('categories_list', 'activation_list', 'level_list', 'challenges'));
    }
