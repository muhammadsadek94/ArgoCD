...
use App\Domains\Course\Rules\KsaPermission;
...
class KsaController extends CoreController
{
    use HasAuthorization;

    public $domain = "course";

    public function __construct(KSA $model)
    {
        $this->model = $model;
        ...
        $this->permitted_actions = [
            'index'  => KsaPermission::KSA_INDEX,
            'create' => KsaPermission::KSA_CREATE,
            'edit'   => KsaPermission::KSA_EDIT,
            'delete' => KsaPermission::KSA_DELETE,
        ];
    }
    ...
    public function onUpdate()
    {
        ...
        $this->validate($this->request, [
            "name"        => ["required", "string", "max:50","unique:{$this->model->getTable()},name,{$this->request->ksa}"],
            'activation'  => ['required', 'boolean'],
        ]);
    }
