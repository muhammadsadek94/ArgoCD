...
class SpecialtyAreaController extends CoreController
{
    use HasAuthorization;

    public $domain = "course";

    public function __construct(SpecialtyArea $model)
    {
        $this->model = $model;
        ...
        $this->permitted_actions = [
            'index'  => SpecialtyAreaPermission::SPECIALTY_AREA_INDEX,
            'create' => SpecialtyAreaPermission::SPECIALTY_AREA_CREATE,
            'edit'   => SpecialtyAreaPermission::SPECIALTY_AREA_EDIT,
            'delete' => SpecialtyAreaPermission::SPECIALTY_AREA_DELETE,
        ];
    }
    ...
    public function onUpdate()
    {
        ...
        $this->validate($this->request, [
            "name"        => ["required", "string", "max:50", "unique:{$this->model->getTable()},name,{$this->request->specialty_area}"],
            'activation'  => ['required', 'boolean'],
        ]);
    }
    ...
    public function searchSpecialtyAreas()
    {
        $search = $this->request->get('search');
        // get all active job roles
        $job_roles = SpecialtyArea::where('name', 'like', "%{$search}%")->active()->get()->map(function ($row) {
            return [
                'id' => $row->id,
                'text' => $row->name
            ];
