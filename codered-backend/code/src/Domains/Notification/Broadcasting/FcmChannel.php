...
class FcmChannel
{
    ...
    public function send($notifiable, Notification $notification)
    {
        ...
        $response_array = [];
        $devices_chunk = $message->getTo()->chunk(1000);

        foreach ($devices_chunk as $devices) {

            foreach($devices as $device) {
                if (is_null($device->device_id)) continue;

                $message->to($device->device_id);
                ...
                $response = $this->client->post(self::API_URI, [
                    ...
                    'body' => $message->formatData($device->language ?? 'en'),
                ]);
