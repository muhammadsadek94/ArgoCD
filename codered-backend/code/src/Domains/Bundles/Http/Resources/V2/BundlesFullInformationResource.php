...
class BundlesFullInformationResource extends JsonResource
{
     /**
    ...
    public function toArray($request)
    {
        ...
        return [
            ...
            'courses_count'             => $this->courses->count() ? $this->courses->count() : 0,
            ...
            'completed_courses_count'   => $this->courses->count() ? $this->completedCoursesCount($request) : 0,
            'completion_percentage'     => $this->courses->count() ? $this->completionPercentage($request) : 0,
            ...
            'jobs'                      => $this->jobs,
            'courses'                   => $this->courses ? CourseForLearnPathExternalResource::collection($this->getCourses()->get()) : [],
            'instructors'               => $this->courses ? InstructorBasicInfoResource::collection($this->getInstructors()) : [],
    ...
    {
        $courses_ids = $this->courses()->orderBy('sort','Asc')->pluck('course_id');
    ...
    private function completedCoursesCount($request) {
        ...
        $courses_ids = $this->getCourses()->pluck('id');
    ...
    private function completionPercentage($request) {
        ...
        $courses_ids = $this->getCourses()->pluck('id');
        $completed_courses = CompletedCourses::where('user_id', $user->id)->whereIn('course_id', $courses_ids)->count();
        return ($completed_courses / $this->getCourses()->count()) * 100;
    }
    ...
    private function getInstructors() {
        $instuctors_ids = [];
        foreach($this->getCourses()->get() as $course){
    ...
    {
        $courses_ids = $this->getCourses()->where('is_free', 0)->active()->pluck('courses.id');
        $chapters = Chapter::whereIn('course_id', $courses_ids)->active()->withCount(['lessons' => function($query){
