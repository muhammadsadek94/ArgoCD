...
class SubaccountTableStatisticsJob extends Job
{
    /**
    ...
    {
        $admin = auth()->user();
        $subaccounts = $user_repository->getSubAccountQuery($request, $admin->id)->pluck('id');
        $user_ids = $user_repository->getSubaccountsUsersQuery($request, $subaccounts)->pluck('id');
        $total_watched_min = $user_repository->getEnterpriseTotalMinWatchedWithTimeFiltration($request, $user_ids);
        ...
        $average_enrolment = $user_repository->getEnterpriseSubAccountAverageEnrollment($request, $admin->id);
